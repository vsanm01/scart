 ...
 *
 * v1.1.3 FIX: getModalContent() now requires and sends a `range` param.
 * GSRECOM's _handleModalContent() has always required e.parameter.range
 * ("Range parameter is required" otherwise) - this was never sent, so every
 * getModalContent() call was returning that error instead of data. Pre-existing
 * bug, unrelated to the v1.4.0 upgrade; fixed while touching this file.
 *
 * v1.1.2 FIX: makePublicRequest() (and therefore getScrollingMessages(),
 * getDoodleEvents(), getCoupons(), getModalContent()) now sends &origin=/
 * &referrer=. GSRECOM v1.4.0 defaults config.enableDomainValidation to true,
 * which checks these params against Sheet1 B2 even on the public,
 * unauthenticated endpoints. Without them, every one of these calls was
 * denied with ERR_SEC_003 as soon as the backend was upgraded to v1.4.0.
 *
 ...
 *   require NO token or signature at all - convenience methods for them
 *   are included below and skip auth entirely, matching the server.
 *
 ...
 *    getModalContent() (public, unsigned, matches the router)
 */
...
(function(window) {
    ...
    GSRCDN.makeRequest = async function(params, options = {}) {
        ...
        try {
            ...
            const _pageOrigin = (window.location.origin && window.location.origin !== 'null') ? window.location.origin : '';
            ...
            if (data.status === 'success') {
                ...
                // DEBUG PATCH: backend error responses use an `error` field (see
                // GSRECOM_v1_4_0_library.js handleGetRequest's catch block), not
                // `message` - so real errors were being swallowed into the generic
                // fallback text below. Checking data.error first surfaces the
                // actual backend reason.
                throw new Error(data.error || data.message || 'GSRCDN: Request failed');
            }
    ...
    // ============================================
    // PUBLIC ENDPOINTS (router-handled, no auth)
    // ============================================
    // These match the public, unauthenticated branches in your Code.gs
    // router exactly: no token, no HMAC signature, no timestamp required.
    // The router only checks `sheet` + `type` query params.
    ...
    GSRCDN.makePublicRequest = async function(params, options = {}) {
        ...
        try {
            ...
            // GSRECOM v1.4.0 defaults enableDomainValidation to true, which
            // checks &origin=/&referrer= against Sheet1 B2 even on the public
            // (unauthenticated) endpoints. Without these, requests get denied
            // with ERR_SEC_003. Guard null origin (file:// pages) the same way
            // makeRequest() does.
            const _pageOrigin = (window.location.origin && window.location.origin !== 'null') ? window.location.origin : '';
            const requestParams = { ...params, origin: _pageOrigin, referrer: _pageOrigin };
    ...
    /**
     * Get scrolling messages (Code.gs router: Sheet3, rows 1-90, ?type=scrolling).
     * @param {string} sheetName - Must match config.scrollingSheet on the server (default 'Sheet3')
     * @returns {Promise<Object>} API response
     */
    GSRCDN.getScrollingMessages = async function(sheetName = 'Sheet3') {
        return GSRCDN.makePublicRequest({ sheet: sheetName, type: 'scrolling' });
    };

    /**
     * Get doodle events (Code.gs router: Sheet3, rows 100+, ?type=doodle).
     * @param {string} sheetName - Must match config.doodleSheet on the server (default 'Sheet3')
     * @returns {Promise<Object>} API response
     */
    GSRCDN.getDoodleEvents = async function(sheetName = 'Sheet3') {
        return GSRCDN.makePublicRequest({ sheet: sheetName, type: 'doodle' });
    };

    /**
     * Get coupons (Code.gs router: Sheet3, rows 150+, ?type=coupons).
     * @param {string} sheetName - Must match config.couponSheet on the server (default 'Sheet3')
     * @returns {Promise<Object>} API response
     */
    GSRCDN.getCoupons = async function(sheetName = 'Sheet3') {
        return GSRCDN.makePublicRequest({ sheet: sheetName, type: 'coupons' });
    };

    /**
     * Get modal content (Code.gs router: Sheet5 or Sheet6, ?type=modal).
     * @param {string} sheetName - Must be one of config.modalSheets on the server ('Sheet5' or 'Sheet6')
     * @param {string} range - Required. A single cell/range (e.g. "A1:B5") or a
     *   comma-separated list of cells (e.g. "A1,B2"). Server returns 'Range
     *   parameter is required' if omitted.
     * @returns {Promise<Object>} API response
     */
    GSRCDN.getModalContent = async function(sheetName, range) {
        if (!sheetName) {
            throw new Error('GSRCDN: getModalContent requires a sheetName ("Sheet5" or "Sheet6")');
        }
        if (!range) {
            throw new Error('GSRCDN: getModalContent requires a range (e.g. "A1:B5" or "A1,B2")');
        }
        return GSRCDN.makePublicRequest({ sheet: sheetName, type: 'modal', range: range });
    };
