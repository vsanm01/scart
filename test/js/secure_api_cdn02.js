 ...
 * built on the SecureSheets library (v3.8.1) + the Code.gs router (v2.0.0).
 *
...
(function(window) {
    ...
    GSRCDN.makeRequest = async function(params, options = {}) {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                // The backend returns errors as { error, code }, not { message }.
                const errMsg = data.error || data.message || 'GSRCDN: Request failed';
                const err = new Error(data.code ? `${errMsg} (${data.code})` : errMsg);
                err.code = data.code;
    ...
    /**
     * NOT SUPPORTED: the SecureSheets backend only implements `getData`.
     * Calling this will always fail server-side ("Unknown action"), so we
     * fail fast client-side instead, with a clearer message.
     */
    GSRCDN.addData = async function() {
        throw new Error('GSRCDN: addData is not supported - the backend (SecureSheets + Code.gs router) only implements the getData action.');
    };

    /** NOT SUPPORTED: see addData. */
    GSRCDN.updateData = async function() {
        throw new Error('GSRCDN: updateData is not supported - the backend (SecureSheets + Code.gs router) only implements the getData action.');
    };

    /** NOT SUPPORTED: see addData. */
    GSRCDN.deleteData = async function() {
        throw new Error('GSRCDN: deleteData is not supported - the backend (SecureSheets + Code.gs router) only implements the getData action.');
    };
    ...
    GSRCDN.makePublicRequest = async function(params, options = {}) {
        ...
        try {
            ...
            if (data.status === 'error' || data.error) {
                const errMsg = data.error || data.message || 'GSRCDN: Public request failed';
                const err = new Error(data.code ? `${errMsg} (${data.code})` : errMsg);
                err.code = data.code;
                err.responseData = data;
                throw err;
     ...
     * @returns {Promise<Object>} API response
     */
    GSRCDN.getModalContent = async function(sheetName) {
        if (!sheetName) {
            throw new Error('GSRCDN: getModalContent requires a sheetName ("Sheet5" or "Sheet6")');
        }
        return GSRCDN.makePublicRequest({ sheet: sheetName, type: 'modal' });
    };
